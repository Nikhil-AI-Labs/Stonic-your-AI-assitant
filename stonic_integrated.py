...
class JarvisIntegratedInterface:
    def __init__(self):
        self.tools = {
    ...
    def start_agent(self):
        ...
        try:
            if agent_process is None or agent_process.poll() is not None:
                logger.info("Starting Stonic agent...")
                agent_process = subprocess.Popen([
                    sys.executable, 'agent.py'
                ], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                jarvis_state['agent_running'] = True
    ...
    def stop_agent(self):
        ...
        try:
            if agent_process and agent_process.poll() is None:
                ...
                agent_process.wait()
                jarvis_state['agent_running'] = False
                socketio.emit('status_update', {
                    'agent_running': False,
    ...
    def process_command(self, command):
        ...
        try:
            ...
            if "start agent" in command_lower or "agent start" in command_lower:
                if self.start_agent():
                    return "Stonic agent is now running and ready for voice interaction."
                else:
                    return "Failed to start the agent. Please check the logs."
            
            if "stop agent" in command_lower or "agent stop" in command_lower:
                if self.stop_agent():
                    return "Stonic agent has been stopped."
                else:
                    return "Failed to stop the agent."
...
jarvis_interface = JarvisIntegratedInterface()
...
def get_status():
    ...
    return jsonify({
        ...
        'agent_running': jarvis_state['agent_running'],
...
def start_agent():
    ...
    try:
        success = jarvis_interface.start_agent()
        return jsonify({
            'success': success,
...
def stop_agent():
    """Stop the LiveKit agent"""
    try:
        success = jarvis_interface.stop_agent()
        return jsonify({
            'success': success,
...
def start_server(host='0.0.0.0', port=5000, debug=False):
    ...
    logger.info(f"Starting Stonic Integrated Web Server on {host}:{port}")
